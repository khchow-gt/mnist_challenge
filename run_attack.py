"""Evaluates a model against examples from a .npy file as specified in config.json"""
...
def run_attack(checkpoint, x_adv, epsilon):
    ...
    with tf.Session() as sess:
        ...
        for ibatch in range(num_batches):
            ...
            cur_corr, y_pred_batch = sess.run([model.num_correct, model.y_pred], feed_dict=dict_adv)
...
if __name__ == '__main__':
    ...
    if checkpoint is None:
        ...
        print('Invalid pixel range. Expected [0, 1], found [{}, {}]'.format(np.amin(x_adv), np.amax(x_adv)))
    else:
